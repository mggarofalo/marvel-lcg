...
public sealed class AbilityRunner(AbilityBook book) : ICardAbilities
{
    ...
    private readonly record struct DamageTransfer(
        ...
        bool PlacesThreat = false, bool FromVillain = false,
        bool ToVillain = false);
        ...
        Card target, Cast cast)
    {
        ...
        foreach (var transfer in trace)
        {
            ...
            int from = transfer.FromVillain ? currentVillain : transfer.From;
            int to = transfer.ToVillain ? currentVillain : transfer.To;
        ...
        AbilityNode node, Cast cast)
    {
        ...
        if (node.Kind == "grantUntil"
            ...
            && Word(granted) == "health"
            && Find(node.Require("card"), cast) is { } healthier)
        {
            return [new DamageTransfer(
