...
internal static class CommandLine
{
        ...
        SimulationConfig config, TextWriter output, TextWriter diagnostics)
    {
        ...
        {
            var streamed = SimulationRunHarness.Run(config, output, diagnostics);
        ...
        {
            summary = SimulationRunHarness.Run(config, records, diagnostics);
        }
    ...
    {
        var summary = SimulationReplayHarness.Replay(config, diagnostics);
        output.WriteLine(
    ...
    private static int Report(string[] args, TextWriter output)
    {
        ...
        output.WriteLine(SimulationReportReader.Report(args[0]).Human(args[0]));
        return 0;
    }

    private static SimulationConfig ParseRun(string[] args) =>
        SimulationRunArguments.Parse(args);
