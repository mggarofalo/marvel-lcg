...
8. **Nothing under `datasets/` may require the network to regenerate.** Every
   dataset is either *generated* (rebuildable offline and byte-identically) or
   *vendored* (copied once from a pinned upstream, read as-is). This makes
   [`marvelcdb`](https://github.com/mggarofalo/marvelcdb-cli) an **acquisition
   and research tool, never a build or engine dependency**.
   `datasets/cards/` and `datasets/setup/` are currently neither — their
   generator is gone and nothing has replaced it. That is a known gap,
   MARVEL-252, not a licence to add a third kind.
