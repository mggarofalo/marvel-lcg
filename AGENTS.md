...
`BotCommand` is the shared plumbing under every policy, and it answers with the **minimum legal** command: the fewest targets, the cheapest payment. That is neutral except where the size of the payment *is* the effect — a printed X (`Variable`) or "spend up to N" (`UpTo`) — and there the minimum is the card doing nothing. Nine cards were inert in every game ever generated because of it, including an ally whose only printed ability is a spend (MARVEL-135). Those two rules now spend maximally, up to the ceiling, in engine order; `-no_bot_pay_variable_cost` restores the old planner, which is what any replay recorded before the change was generated under. **An unmoved digest does not clear a change like this**: none of the seven wide-matrix cases reach a variable cost at all, so the bound came from six constructed ones, of which two moved.
