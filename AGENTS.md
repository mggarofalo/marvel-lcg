...
8. **Nothing under `datasets/` may require the network to regenerate**, and
   ...
   - *Generated* — rebuildable offline and byte-identically, with a `--check`
     ...
     CI runs `-- check` once on pull requests and on both platform legs for
     `master` and releases, so a hand edit is a red build.
...
| Workflow | Runs | What |
|---|---|---|
| [`ci.yml`](.github/workflows/ci.yml) | every push to `master`, every PR, every release | managed correctness once on PRs; platform integration on Windows and Linux; exhaustive cross-platform certification on `master` and releases |

**Every platform-bound gate in `ci.yml` is verified green on Windows and
Linux.** Platform-neutral checks run once on pull requests and on both legs for
`master` and protected releases. Keep it that way: a platform gate that has
never passed on one OS does not belong here. A red `master` must mean something
broke.
