...
8. **Nothing under `datasets/` may require the network to regenerate.** Every
   ...
   *vendored* (copied once from a pinned upstream, read as-is). This makes
   ...
   `datasets/cards/` is generated by `tools/Marvel.Cards.Extract` from the
   vendored MarvelSDB snapshot and `datasets/cards/supplement.json`; CI runs
   `-- check` on both legs, so a hand edit is a red build.
   `datasets/setup/` is still neither, and has no upstream to be generated
   from — MARVEL-252. That is a known gap, not a licence to add a third kind.
