...
`unit_test/test_integrity_errors.py` enforces that rule rather than restating it. It walks `engine/` and `game/` and fails on any broad `except` that swallows — unless the handler re-raises, reports through `Log.OnCrash`, or sits behind an earlier `except EngineIntegrityError`. Everything that legitimately absorbs is listed in `REVIEWED_ABSORBERS` with the reason. **Adding an entry is a decision:** if an integrity error can reach the `try`, put `except EngineIntegrityError: raise` ahead of the broad clause instead — the scan goes quiet either way and only one of the two is correct.
