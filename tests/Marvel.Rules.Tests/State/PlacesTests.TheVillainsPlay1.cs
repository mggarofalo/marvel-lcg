...
public sealed class PlacesTheVillainsPlayTests : PlacesTestBase
{
    // ---------------------------------------------------------------- rr:play-area
    [Rule("rr:play-area")]
    [Rule("rr:play-area.2")]
    ...
    public void TheVillainsPlayAreaIsAPlaceAndNotTheAbsenceOfOne()
    {
        ...
        DeckType[] villainAreas = [DeckType.VillainArea, DeckType.VillainDeck, DeckType.MainSchemesArea, DeckType.MainSchemesDeck, DeckType.EncounterDeck, DeckType.EncounterDiscardPile, ];
    ...
    public void JoiningAGameAreaFromAnotherWorldIsRefused()
    {
        ...
        Assert.Throws<ArgumentException>(() => world.Join(PlayArea.Of(0), stranger, "test", events));
        Assert.Empty(events);
    }
