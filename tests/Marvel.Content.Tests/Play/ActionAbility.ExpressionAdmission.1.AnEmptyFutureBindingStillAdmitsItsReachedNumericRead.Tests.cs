...
public sealed class ActionAbilityExpressionAdmissionAnEmptyFutureBindingStillAdmitsItsReachedNumericReadTests
{
    [Fact]
    ...
    public void NumericBranchesAdmitOnlyTheSingleCardReadsTheyReach(bool readsDiscard)
    {
        ...
        var runner = Runner(AuthoredCards.AuntMay, "Action", $$$$$$"""
            ...
            """, cost: """{"exhaust":"this"}""");
        var(world, source) = FixedCountBoard(runner);
        var minion = world.CreateCard(AuthoredCards.Shocker, world.AreaOf(DeckType.EngagedEnemiesArea, PlayArea.Of(0)));
        var discarded = world.CreateCard(AuthoredCards.Shocker, world.AreaOf(DeckType.EncounterDiscardPile));
        string before = world.Digest().Canonical();
        if (readsDiscard)
        {
            var error = Assert.Throws<RulesNotImplementedException>(() => runner.Actions(world, 0));
