...
public sealed class ActionAbilityCompiledTreeAContinuationFrameMustIdentifyAChildOfTheCompiledInstrucTests
{
    [Theory]
    ...
    public void AContinuationFrameMustIdentifyAChildOfTheCompiledInstruction(string frame)
    {
        ...
        var forged = new PhaseStep(Steps.ResumeAbility, 1, 2, Subject: source.ObjectId, Seat: 0, Tier: AbilityType.Action, AbilityOrdinal: 0, AbilityPath: [frame], AbilityFace: source.FaceId, AbilityHasContinuation: true);
    ...
    public void PlayerQualifiedDiscardChoiceChecksTheSelectedDeck(bool selectedDeckEmpty)
    {
        ...
        var selection = Assert.IsType<Prompt>(Sequence.Work(world, CardCatalogData, runner, []));
        Sequence.Answer(world, CardCatalogData, runner, selection, Decision.Take(world.Seats[1].IdentityCard.ObjectId), []);
        var options = Assert.IsType<Prompt>(Sequence.Work(world, CardCatalogData, runner, []));
        ...
        Sequence.Answer(world, CardCatalogData, runner, options, Decision.Take(selectedDeckEmpty ? 1 : 0), []);
    ...
    public void CompiledContinuationDoesNotRetainTheSuppliedBookOrSequence(bool afterSuspension)
    {
        ...
        {
            parsed.Abilities[0] with
            {
                Effect = new AbilityNode("seq", new AbilityValue.List(steps))
            },
        };
