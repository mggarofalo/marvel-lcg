...
public sealed class ActionAbilityTypedRepetitionMutableCountUnderArithmeticTests
{
    [Theory]
    [InlineData("add")]
    [InlineData("mul")]
    ...
    public void StableArithmeticCountDoesNotBecomeMutableAfterCompilation()
    {
        ...
        fields["count"] = new AbilityValue.Map(new Dictionary<string, AbilityValue> { ["damageOn"] = new AbilityValue.Word("you"), });
        game.Resolve(Decision.Take(action.Id));
    ...
    {
        string arguments = operation == "dealDamage" ? """{"cards":{"query":"villain"},"amount":1}""" : """{"scheme":{"query":"mainScheme"},"amount":1}""";
        var(runner, fields) = MutableEffectRunner("forEach", $$"""{"count":3,"effect":{"{{operation}}": {{arguments}} } }""", false);
        Card? source = null;
        var(game, world) = Playing(board =>
        {
            source = InPlay(board, AuthoredCards.AuntMay);
        ...
        var changed = operation == "dealDamage" ? world.TheCardIn(DeckType.VillainArea)! : world.TheCardIn(DeckType.MainSchemesArea)!;
    ...
    public void EachTimeKeepsCompiledDiscardAndPredicateAcrossChoices(bool changeWhilePending)
    {
        ...
        {
            fields["effect"] = new AbilityValue.Map(new Dictionary<string, AbilityValue> { ["discardTop"] = new AbilityValue.Map(new Dictionary<string, AbilityValue> { ["from"] = new AbilityValue.Word("yourDeck"), ["count"] = new AbilityValue.Number(1), }), });
            fields["when"] = new AbilityValue.Map(new Dictionary<string, AbilityValue> { ["isKind"] = new AbilityValue.Map(new Dictionary<string, AbilityValue> { ["card"] = new AbilityValue.Word("that"), ["kind"] = new AbilityValue.Word("treachery"), }), });
        }
