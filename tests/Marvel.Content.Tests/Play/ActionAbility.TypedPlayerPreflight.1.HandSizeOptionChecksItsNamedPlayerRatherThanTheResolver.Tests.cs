...
public sealed class ActionAbilityTypedPlayerPreflightHandSizeOptionChecksItsNamedPlayerRatherThanTheResolverTests
{
    [Theory]
    ...
    public void HandSizeOptionChecksItsNamedPlayerRatherThanTheResolver(string operation, bool full)
    {
        ...
        for (int index = 0; index < held; index++)
            world.CreateCard("01002", recipient.Hand);
        ...
        {
            Assert.Throws<RulesNotImplementedException>(() => runner.Chose(world, source, 0, choice.Index, Decision.Take(0), choice.Tier));
            Assert.Equal(size, recipient.Hand.Cards.Count);
    ...
    {
        var(runner, fields) = MutableNumericRunner("discardAtRandom", $$"""{"player":"{{players}}","count":1}""", choice: true);
        var(world, source) = FixedCountBoard(runner, players: 2);
        world.FirstPlayer = 1;
    ...
    public void APostCompilationFieldCannotRedirectAChoiceToAnotherPlayer(bool cards)
    {
        ...
        string arguments = cards ? """{"from":{"query":"villain"},"effect":{"heal":{"card":"chosen","amount":1}}}""" : """
