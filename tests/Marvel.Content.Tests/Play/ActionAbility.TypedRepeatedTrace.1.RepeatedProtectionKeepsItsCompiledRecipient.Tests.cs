...
public sealed class ActionAbilityTypedRepeatedTraceRepeatedProtectionKeepsItsCompiledRecipientTests
{
    [Theory]
    [InlineData("heal")]
    ...
    {
        string grant = grantKind == "trait" ? """{"card":{"titled":"Shocker"},"trait":"AERIAL","until":"EndOfRound"}""" : """{"card":{"titled":"Shocker"},"keyword":"attack","amount":3,"until":"EndOfRound"}""";
        string selection = grantKind == "trait" ? """{"withTrait":{"cards":{"query":"minions"},"trait":"AERIAL"}}""" : """{"maxBy":{"of":{"query":"enemies"},"by":"attack"}}""";
        var(runner, fields) = MutableRepeatedTraceRunner($$$"""
            [{"grantUntil":{{{grant}}}},
        ...
        fields["card"] = new AbilityValue.Word("this");
        if (grantKind == "trait")
            fields["trait"] = new AbilityValue.Word("AVENGER");
        else
            fields["keyword"] = new AbilityValue.Word("scheme");
        Card? source = null;
        Card? minion = null;
        World? world = null;
    ...
    {
        string selection = selectionKind == "titled" ? """{"titled":"Rhino"}""" : """{"minBy":{"of":{"query":"enemies"},"by":"attack"}}""";
        var(runner, fields) = MutableRepeatedTraceRunner($$$"""
            [{"dealDamage":{"cards":{"query":"villain"},"amount":100}},
             {"dealDamage":{"cards":{{{selection}}},"amount":1}},
             {"moveDamage":{"from":{"query":"villain"},"to":{"titled":"Spider-Man"},"amount":1}}]
            """, 1);
        fields["cards"] = new AbilityValue.Map(new Dictionary<string, AbilityValue> { ["query"] = new AbilityValue.Word("villain"), });
        Card? source = null;
        var(game, world) = Playing(board =>
        {
            source = InPlay(board, AuthoredCards.AuntMay);
            if (selectionKind == "titled")
