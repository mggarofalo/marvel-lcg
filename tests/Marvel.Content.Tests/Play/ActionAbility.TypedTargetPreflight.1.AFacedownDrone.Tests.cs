...
public sealed class ActionAbilityTypedTargetPreflightAFacedownDroneTests
{
    ...
    public void TargetEligibilityAndExecutionUseTheCompiledSelection(string operation, bool choice)
    {
        ...
        Assert.Equal(operation is "dealDamage" or "dealAttackDamage" ? 1 : 0, world.TheCardIn(DeckType.VillainArea)!.Damage);
        Assert.Equal(beforeThreat + (operation == "placeThreat" ? 1 : operation == "removeThreat" ? -1 : 0), scheme.Tokens.GetValueOrDefault("k_threat"));
        Assert.Equal(operation == "giveStatus" ? 1 : 0, Statuses.Count(world, identity, Statuses.Stunned));
        Assert.Equal(beforeHealth + (operation == "grantUntil" ? 1 : 0), Damage.Health(world, world.Facts, identity));
    ...
    {
        var(runner, fields) = MutableNumericRunner("removeThreat", """{"scheme":{"query":"mainScheme"},"amount":1,"ignoresCrisis":"true"}""", choice);
    ...
    public void ThreatRemovalPreflightOverridesOnlyItsCompiledSource(bool choice, bool anotherProhibition)
    {
        ...
        if (choice)
        {
            var fallback = AbilityCatalog.Parse("""
                ...
                """).Abilities[0].Effect;
            effect = new AbilityNode("choose", new AbilityValue.Map(new Dictionary<string, AbilityValue> { ["options"] = new AbilityValue.List([new AbilityValue.Map(new Dictionary<string, AbilityValue> { [effect.Kind] = effect.Argument }), new AbilityValue.Map(new Dictionary<string, AbilityValue> { [fallback.Kind] = fallback.Argument }), ]), }));
        }

        var abilities = parsed.Abilities.ToList();
        abilities[1] = action with
        {
            Effect = effect
        };
    ...
    public void PowerProjectionUsesCompiledRecipientsBeforePredictingElimination(string operation, bool dependent)
    {
        ...
        steps[0] = new AbilityValue.Map(new Dictionary<string, AbilityValue> { [operation] = new AbilityValue.Map(fields), });
        if (dependent)
        {
            steps[0] = new AbilityValue.Map(new Dictionary<string, AbilityValue> { ["otherwise"] = new AbilityValue.Map(new Dictionary<string, AbilityValue> { ["effect"] = steps[0], ["otherwise"] = new AbilityValue.Map(new Dictionary<string, AbilityValue> { ["dealDamage"] = new AbilityValue.Map(new Dictionary<string, AbilityValue> { ["cards"] = new AbilityValue.Word("you"), ["amount"] = new AbilityValue.Number(1), }), }), }), });
        }

        powerFields["effect"] = new AbilityValue.Map(new Dictionary<string, AbilityValue> { ["seq"] = new AbilityValue.List(steps), });
        var runner = new AbilityRunner(new AbilityBook([parsed.Abilities[0] with { Effect = new AbilityNode("attack", new AbilityValue.Map(powerFields)) }], parsed.Authored));
