...
public sealed class ActionAbilityTypedAreaProjectionProjectedAttackableSelectionExcludesTheVillainWhileGuardTests
{
    [Rule("rr:guard.1")]
    [Fact]
    public void ProjectedAttackableSelectionExcludesTheVillainWhileGuardRemains()
    {
        ...
        var attachment = world.CreateCard("01098", world.AreaOf(DeckType.UpgradesArea, villain.Area.PlayArea, villain.ObjectId));
    ...
    public void AreaProjectionDoesNotTreatAnUnknownConditionAsFalse(string wrapper)
    {
        ...
        string condition = wrapper == "not" ? """{"not":{"inForm":{"player":"you","form":"alter-ego"}}}""" : $$"""{"{{wrapper}}":[{{status}},{{hero}}]}""";
        var(runner, _) = MutableAreaSequenceRunner($$"""
            [{"if":{"test":{{condition}},
        ...
        if (wrapper == "and")
            Statuses.Give(world, minion, Statuses.Stunned);
    ...
    public void AreaProjectionKeepsTheCompiledStatusCondition(string wrapper, bool drawOnTrue)
    {
        ...
        fields["test"] = new AbilityValue.Map(new Dictionary<string, AbilityValue> { ["not"] = fields["test"], });
        var action = Assert.Single(runner.Actions(world, 0), option => option.Card == source.ObjectId);
        runner.Act(world, action, [], []);
        Assert.Equal(damageBefore, minion.Damage);
        Assert.Equal(DeckType.EngagedEnemiesArea, minion.Area.Type);
