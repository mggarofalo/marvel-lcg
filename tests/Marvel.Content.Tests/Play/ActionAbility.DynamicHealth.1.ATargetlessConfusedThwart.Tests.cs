...
public sealed class ActionAbilityDynamicHealthATargetlessConfusedThwartTests
{
    ...
    public void ATargetlessConfusedThwartIsOfferedAndResolvesThroughTheGame()
    {
        ...
        {
            foreach (var scheme in board.Cards.Where(card => card.Area.Type is DeckType.MainSchemesArea or DeckType.SideSchemesArea))
            {
                scheme.PlaceTokens("k_threat", -scheme.Tokens.GetValueOrDefault("k_threat"));
            }

            Statuses.Give(board, board.Seats[0].IdentityCard, Statuses.Confused);
        }, hero: true, abilities: AuthoredCards.Runner());
        var thwart = Assert.Single(game.Pending!.Affordances, option => option.Verb == BasicPowers.ThwartVerb && option.AnchorId == world.Seats[0].IdentityCard.ObjectId);
        Assert.Empty(thwart.Targets!.Legal);
        Assert.Equal(0, thwart.Targets.Min);
        Assert.Equal(0, thwart.Targets.Max);
        Assert.Throws<RulesNotImplementedException>(() => game.Resolve(Decision.Take(thwart.Id, [world.TheCardIn(DeckType.VillainArea)!.ObjectId], [])));
    ...
    public void AnOptionalHealthLossInterruptPausesAndThenResumesTheAbility()
    {
        ...
        var(game, world) = Playing(board =>
        {
            ...
            board.CreateCard("01163", board.AreaOf(DeckType.UpgradesArea, minion.Area.PlayArea, minion.ObjectId));
            board.CreateCard("01185", board.AreaOf(DeckType.UpgradesArea, minion.Area.PlayArea, minion.ObjectId));
            minion.TakeDamage(3);
        }, hero: true, abilities: abilities);
        var scheme = world.TheCardIn(DeckType.MainSchemesArea)!;
        long threatBefore = scheme.Tokens.GetValueOrDefault("k_threat");
        Assert.Equal(6, Damage.Health(world, CardCatalogData, minion!));
        var action = Assert.Single(game.Pending!.Affordances, option => option.AnchorId == source!.ObjectId && option.Verb == Game.ActionVerb);
        ...
        Assert.Equal(threatBefore + 1, scheme.Tokens.GetValueOrDefault("k_threat"));
    ...
    public void StartingHealthExcludesAHitPointModifier()
    {
        ...
        var(game, world) = Playing(board =>
        {
            ...
            board.Effects.Register(new ContinuousEffect(EffectSource.LastingEffect, "health", Amount: 6, Card: source.ObjectId, Affects: hero.ObjectId));
        }, hero: true, abilities: runner);
        var action = Assert.Single(game.Pending!.Affordances, option => option.AnchorId == source!.ObjectId && option.Verb == Game.ActionVerb);
        game.Resolve(Decision.Take(action.Id));
        Assert.Equal(10, world.Seats[0].IdentityCard.Damage);
        Assert.False(world.Seats[0].Eliminated);
        Assert.Equal(6, Damage.Health(world, CardCatalogData, world.Seats[0].IdentityCard) - world.Seats[0].IdentityCard.Damage);
    }
}
