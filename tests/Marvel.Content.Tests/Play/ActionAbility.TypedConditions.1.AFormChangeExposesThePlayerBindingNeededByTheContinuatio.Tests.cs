...
public sealed class ActionAbilityTypedConditionsAFormChangeExposesThePlayerBindingNeededByTheContinuatioTests
{
    [Fact]
    public void AFormChangeExposesThePlayerBindingNeededByTheContinuation()
    {
        ...
        var second = Assert.Single(game.Pending!.Affordances, option => option.Id == world.Seats[1].IdentityCard.ObjectId);
    ...
    public void ConditionalBindsTheSelectedPlayerBeforeTesting(string wrapper, int drawn)
    {
        ...
        var action = Assert.Single(game.Pending!.Affordances, option => option.AnchorId == source!.ObjectId);
        game.Resolve(Decision.Take(action.Id));
        var option = Assert.Single(game.Pending!.Affordances, candidate => candidate.Id == world.Seats[1].IdentityCard.ObjectId);
    ...
    public void ConditionalKeepsItsCompiledTestAcrossChoices(bool hero, bool repeated, bool changeWhilePending)
    {
        ...
        var action = Assert.Single(game.Pending!.Affordances, option => option.AnchorId == source!.ObjectId);
        void ChangeTest() => fields["test"] = new AbilityValue.Map(new Dictionary<string, AbilityValue> { ["inForm"] = new AbilityValue.Map(new Dictionary<string, AbilityValue> { ["player"] = new AbilityValue.Word("you"), ["form"] = new AbilityValue.Word("alter-ego"), }), });
