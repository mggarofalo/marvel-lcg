...
public sealed class ActionAbilityTypedDamageCompiledThreatExceptionOverridesOnlyItsNamedSourceTests
{
    [Theory]
    [InlineData(false)]
    [InlineData(true)]
    public void CompiledThreatExceptionOverridesOnlyItsNamedSource(bool anotherProhibition)
    {
        ...
        abilities[1] = abilities[1] with
        {
            Effect = new AbilityNode("removeThreat", new AbilityValue.Map(fields))
        };
    ...
    public void AttackDamageKeepsItsCompiledOverkillFlag(bool overkill)
    {
        ...
        var(runner, fields) = MutableEffectRunner("dealAttackDamage", $$"""{"cards":{"query":"minionsEngagedWithYou"},"amount":5{{marker}}}""", false);
        Card? source = null;
        Card? minion = null;
        ...
        long excess = 5 - Damage.Health(world, CardCatalogData, minion!);
        Assert.True(excess > 0);
        var action = Assert.Single(game.Pending!.Affordances, option => option.AnchorId == source!.ObjectId);
        if (overkill)
            fields.Remove("overkill");
        else
            fields["overkill"] = new AbilityValue.Number(1);
    ...
    public void OrdinaryDamageKeepsItsCompiledEventVerb(bool attackVerb)
    {
        ...
        var(runner, fields) = MutableEffectRunner("dealDamage", $$"""{"cards":{"query":"villain"},"amount":1{{marker}}}""", false);
        Card? source = null;
        ...
        if (attackVerb)
            ...
            fields["attack"] = new AbilityValue.Number(1);
    ...
    public void DamageAndThreatExecutionUsesCompiledAmounts(string operation, bool repeated)
    {
        ...
        {
            "moveDamage" or "moveAttackDamage" => """{"from":"you","to":{"query":"villain"},"amount":1}""",
        ...
        Assert.Equal(operation is "placeThreat" or "removeThreat" ? 0 : amount, world.TheCardIn(DeckType.VillainArea)!.Damage);
        Assert.Equal(operation is "moveDamage" or "moveAttackDamage" ? 4 : 5, world.Seats[0].IdentityCard.Damage);
        Assert.Equal(operation == "placeThreat" ? 4 : operation == "removeThreat" ? 3 - amount : 3, world.TheCardIn(DeckType.MainSchemesArea)!.Tokens.GetValueOrDefault("k_threat"));
    ...
    {
        var(runner, fields) = MutableEffectRunner("indirectDamage", """{"among":{"query":"heroesAndAllies"},"amount":2}""", repeated: false);
        Card? source = null;
        Card? ally = null;
        var(game, world) = Playing(board =>
        {
            source = InPlay(board, AuthoredCards.AuntMay);
            ally = board.CreateCard("01020", board.AreaOf(DeckType.AlliesArea, PlayArea.Of(0), cardOwner: 0));
        }, hero: true, abilities: runner);
