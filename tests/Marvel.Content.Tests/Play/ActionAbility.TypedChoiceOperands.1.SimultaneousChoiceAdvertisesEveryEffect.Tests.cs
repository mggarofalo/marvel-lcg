...
public sealed class ActionAbilityTypedChoiceOperandsSimultaneousChoiceAdvertisesEveryEffectTests
{
    [Fact]
    public void SimultaneousChoiceAdvertisesEveryEffectAndExecutesTheChosenOrder()
    {
        ...
        runner.Chose(world, source, 0, choice.Index, Decision.Take(order.Id, [1, 0, 2], []), choice.Tier);
    ...
    {
        var(runner, fields) = MutableEffectRunner(operation, """{"resources":"BB","otherwise":{"exhaust":"you"}}""", false);
        var(world, source) = FixedCountBoard(runner);
        ...
        Assert.Throws<RulesNotImplementedException>(() => runner.Chose(world, source, 0, choice.Index, Decision.Take(0, [], [energy.ObjectId]), choice.Tier));
        Assert.Equal([genius, energy], hand.Cards);
        runner.Chose(world, source, 0, choice.Index, Decision.Take(0, [], [genius.ObjectId]), choice.Tier);
    ...
    public void CardChoicesKeepTheirCompiledCandidatesAndConcealment(string area, string wrapper, bool concealed)
    {
        ...
        var(runner, fields) = MutableEffectRunner("chooseCard", $$$"""{"from":{{{selection}}},"effect":{"addToHand":"chosen"}}""", false);
        var(world, source) = FixedCountBoard(runner);
        ...
        Assert.Throws<RulesNotImplementedException>(() => runner.Chose(world, source, 0, choice.Index, Decision.Take(world.Seats[0].IdentityCard.ObjectId), choice.Tier));
    ...
    {
        var(runner, fields) = MutableEffectRunner("resolveSpecials", """{"cards":{"query":"upgradesYouControl"}}""", false);
        var(world, source) = FixedCountBoard(runner);
    ...
    public void ThwartChoicesOfferAndValidateTheCompiledSchemes(string operation)
    {
        ...
        var prompt = Sequence.Work(world, CardCatalogData, runner, [])!;
        var selection = Assert.Single(prompt.Affordances);
        if (operation == "legalPractice")
            Assert.Equal(side.ObjectId, selection.Id);
        else
            Assert.Equal([side.ObjectId], selection.Targets!.Legal);
        var wrong = operation == "legalPractice" ? Decision.Take(main.ObjectId, [handCard.ObjectId], []) : Decision.Take(selection.Id, [main.ObjectId], []);
        Assert.Throws<RulesNotImplementedException>(() => runner.Chose(world, source, 0, choice.Index, wrong, choice.Tier));
        Assert.Equal([handCard], world.Seats[0].Hand.Cards);
        Assert.Equal(4, main.Tokens["k_threat"]);
        Assert.Equal(3, side.Tokens["k_threat"]);
        var answer = operation == "legalPractice" ? Decision.Take(side.ObjectId, [handCard.ObjectId], []) : Decision.Take(selection.Id, [side.ObjectId], []);
        Sequence.Answer(world, CardCatalogData, runner, prompt, answer, []);
        Sequence.Finish(world, CardCatalogData, runner, []);
        Assert.Equal(4, main.Tokens["k_threat"]);
