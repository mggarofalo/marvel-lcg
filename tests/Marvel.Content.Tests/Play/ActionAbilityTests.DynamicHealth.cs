...
public sealed partial class ActionAbilityTests
{
    ...
    [Rule("rr:stalwart.2")]
    [Fact]
    public void AConditionalStalwartGrantRemovesAnExistingStatusImmediately()
    {
        // Unus gains Stalwart at six threat on Gene Pool. Raising the scheme
        // from five to six makes that constant active, and "each stunned
        // and/or confused status card is removed" before later game text can
        // inspect the newly stalwart board.
        var runner = Runner(
        ...
        var (game, world) = Playing(
            board =>
            {
                ...
                pool.PlaceTokens("k_threat", 5);
                unus = board.CreateCard(
                    "45059",
                    board.AreaOf(DeckType.EngagedEnemiesArea, PlayArea.Of(0)));
                Statuses.Give(board, unus, Statuses.Stunned);
            },
            hero: true,
            abilities: runner);
        ...
        Assert.False(Statuses.Has(world, unus!, Statuses.Stunned));
    }

    [Rule("rr:hit-points.3.1")]
    [Fact]
    public void AConditionalHealthGrantEndingDefeatsTheNowLethalMinion()
    {
        // Infinite Soldier has +3 HP at nine Gene Pool threat. Thwarting to
        // eight switches that constant off; its three damage now equal its
        // printed three HP, so the minion is defeated immediately.
        var runner = Runner(
            AuthoredCards.AuntMay,
            "Action",
            """
