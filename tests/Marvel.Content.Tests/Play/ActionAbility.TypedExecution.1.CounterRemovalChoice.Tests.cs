...
public sealed class ActionAbilityTypedExecutionCounterRemovalChoiceTests
{
    [Theory]
    [InlineData(true)]
    [InlineData(false)]
    public void CounterRemovalChoiceUsesCompiledEligibilityAndExecution(bool mutateBeforeAction)
    {
        ...
        var fields = ((AbilityValue.Map)original.Argument).Entries.ToDictionary(pair => pair.Key, pair => pair.Value, StringComparer.Ordinal);
        var changedOption = new AbilityValue.Map(new Dictionary<string, AbilityValue>(StringComparer.Ordinal) { ["removeCounters"] = new AbilityValue.Map(fields), });
        var choice = new AbilityNode("choose", new AbilityValue.Map(new Dictionary<string, AbilityValue>(StringComparer.Ordinal) { ["options"] = new AbilityValue.List([changedOption, options.Values[1]]), }));
        var runner = new Marvel.Cards.Run.AbilityRunner(new AbilityBook([parsed.Abilities[0] with { Effect = choice }], parsed.Authored));
        Card? source = null;
    ...
    public void MaintenanceEffectsUseCompiledValuesAndExpiry(string changedOperation)
    {
        ...
        var fields = nodes.ToDictionary(node => node.Kind, node => ((AbilityValue.Map)node.Argument).Entries.ToDictionary(pair => pair.Key, pair => pair.Value, StringComparer.Ordinal), StringComparer.Ordinal);
        var effect = new AbilityNode("seq", new AbilityValue.List(nodes.Select(node => (AbilityValue)new AbilityValue.Map(new Dictionary<string, AbilityValue>(StringComparer.Ordinal) { [node.Kind] = new AbilityValue.Map(fields[node.Kind]), })).ToList()));
        var runner = new Marvel.Cards.Run.AbilityRunner(new AbilityBook([parsed.Abilities[0] with { Effect = effect }], parsed.Authored));
        Card? source = null;
        var(game, world) = Playing(board =>
        {
            source = InPlay(board, AuthoredCards.AuntMay);
        ...
        long attack = StateFields.Modified(world, identity, "attack", CardCatalogData, world.Players);
        var action = Assert.Single(game.Pending!.Affordances, option => option.AnchorId == source!.ObjectId);
        ...
        Assert.Equal(attack + 2, StateFields.Modified(world, identity, "attack", CardCatalogData, world.Players));
        world.Effects.Expire(TimingPoints.EndOfPhase);
        Assert.Equal(attack + 2, StateFields.Modified(world, identity, "attack", CardCatalogData, world.Players));
        world.Effects.Expire(TimingPoints.EndOfRound);
        Assert.Equal(attack, StateFields.Modified(world, identity, "attack", CardCatalogData, world.Players));
    }
    ...
    public void HandSizeDrawDistinguishesPrintedAndModifiedValues(string operation, int expected)
    {
        ...
        var runner = Runner(AuthoredCards.AuntMay, "Action", $$"""{"{{operation}}":"you"}""", cost: """{"exhaust":"this"}""");
        Card? source = null;
        var(game, world) = Playing(board => source = InPlay(board, AuthoredCards.AuntMay), abilities: runner);
        Hand(world, Physicals, 1);
        var identity = world.Seats[0].IdentityCard;
        world.Effects.Register(new ContinuousEffect(EffectSource.LastingEffect, Kind: "hand_size", Amount: 3, Card: source!.ObjectId, Affects: identity.ObjectId));
        var action = Assert.Single(game.Pending!.Affordances, option => option.AnchorId == source.ObjectId);
        game.Resolve(Decision.Take(action.Id));
        Assert.False(source.Ready);
    ...
    public void ImmediateEffectsResumeWithCompiledArguments(bool mutateBeforeAction)
    {
        var parsed = AbilityCatalog.Parse("""
            ...
            """);
        static AbilityValue.Map Operation(string name, AbilityValue argument) => new(new Dictionary<string, AbilityValue>(StringComparer.Ordinal) { [name] = argument });
        ...
        var effect = new AbilityNode("seq", new AbilityValue.List([Operation("draw", new AbilityValue.Map(firstDraw)), Operation("choose", new AbilityValue.Map(new Dictionary<string, AbilityValue>(StringComparer.Ordinal) { ["options"] = new AbilityValue.List([Operation("draw", new AbilityValue.Map(firstDraw)), Operation("draw", new AbilityValue.Map(lastDraw)), ]), })), Operation("placeCounters", new AbilityValue.Map(counters)), Operation("draw", new AbilityValue.Map(lastDraw)), ]));
