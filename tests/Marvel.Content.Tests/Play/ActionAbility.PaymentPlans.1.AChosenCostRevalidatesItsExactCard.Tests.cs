...
public sealed class ActionAbilityPaymentPlansAChosenCostRevalidatesItsExactCardTests
{
    ...
    public void AChosenCostRevalidatesItsExactCardBeforeAnyCostIsPaid(string selection)
    {
        ...
        var(_, world) = Playing(board =>
        {
            ...
            exhausted.Exhaust();
            foreign = board.CreateCard(AuthoredCards.BlackCat, board.AreaOf(DeckType.AlliesArea, PlayArea.Of(1), cardOwner: 1));
        }, heroes: ["spider_man", "captain_marvel"], abilities: runner);
        var ability = Assert.Single(runner.Actions(world, 0), candidate => candidate.Card == source!.ObjectId);
        ...
        if (selection != "legal")
        {
            Assert.Throws<RulesNotImplementedException>(() => runner.Act(world, ability, [], [selected.ObjectId]));
    ...
    public void OfferingASimultaneousResourceCostRequiresEveryComponent(bool hasPhysical)
    {
        ...
        var runner = Runner(AuthoredCards.AuntMay, "Action", """{"draw":{"player":"you","count":1}}""", cost: """{"seq":[{"spend":"R"},{"spend":"Y"}]}""");
        Card? source = null;
        Card? energy = null;
        var(_, world) = Playing(board =>
        {
    ...
    public void AnEventPaymentValidatesAllAllocationsBeforeItsOtherCosts(bool valid)
    {
        ...
        var runner = Runner(AuthoredCards.Backflip, "Action", """{"draw":{"player":"you","count":1}}""", cost: """{"seq":[{"spend":"Y"},{"exhaust":"you"}]}""");
        Card? source = null;
        Card? generator = null;
        var(_, world) = Playing(board =>
        {
        ...
        IReadOnlyList<ResourceAllocation> allocation = [new(generator!.ObjectId, Cost: valid ? 1 : 0, PaidAs: "Y")];
        if (!valid)
        {
            Assert.Throws<RulesNotImplementedException>(() => runner.Act(world, ability, [generator.ObjectId], [], occurrence, allocations: allocation));
            Assert.Equal(before, world.Digest().Canonical());
            Assert.Equal(words, world.Random.Generator.WordsConsumed);
        ...
        var events = runner.Act(world, ability, [generator.ObjectId], [], occurrence, allocations: allocation);
        ...
        int eventPlaced = events.ToList().FindIndex(happened => happened is CardsMoved moved && moved.Cards.Any(landing => landing.Card == source.ObjectId));
        int generatorSpent = events.ToList().FindIndex(happened => happened is CardsMoved moved && moved.Cards.Any(landing => landing.Card == generator.ObjectId));
        int exhausted = events.ToList().FindIndex(happened => happened is FieldSet field && field.Card == identity.ObjectId && field.Field == "is_exhaust");
    ...
    public void DamagePaymentDistinguishesUnpaidTakingFromPaidDealing(bool mustTakeAll)
    {
        ...
        int held = world.Seats[0].Hand.Cards.Count;
        long words = world.Random.Generator.WordsConsumed;
        world.Effects.Register(new ContinuousEffect(EffectSource.LastingEffect, Kind: "preventDamage", Amount: 1, Card: source!.ObjectId, Affects: identity.ObjectId, Lasts: new Duration(Uses: 1)));
        if (mustTakeAll)
        {
            var failure = Assert.Throws<RulesNotImplementedException>(() => runner.Act(world, ability, [generator!.ObjectId], []));
