...
public sealed class ActionAbilityTypedActivationsActivationKeepsCompiledPriorityTests
{
    [Theory]
    [InlineData(true)]
    [InlineData(false)]
    ...
    {
        var(runner, fields) = MutableEffectRunner("enemyAttacks", $$"""{"enemies":{"query":"minionsEngagedWithYou"},"first":"{{(first ? "true" : "false")}}"}""", false);
        Card? source = null;
        Card? minion = null;
        var(_, world) = Playing(board =>
        {
            source = InPlay(board, AuthoredCards.AuntMay);
            minion = board.CreateCard(AuthoredCards.Shocker, board.AreaOf(DeckType.EngagedEnemiesArea, PlayArea.Of(0)));
        }, abilities: runner);
        var action = Assert.Single(runner.Actions(world, 0), ability => ability.Card == source!.ObjectId);
        var villain = world.TheCardIn(DeckType.VillainArea)!;
        world.Agenda.NowActivation(new PhaseStep(Steps.Attack, 1, 2, Subject: villain.ObjectId, Seat: 0));
        ...
        Assert.Equal(first ? [minion!.ObjectId, villain.ObjectId] : [villain.ObjectId, minion!.ObjectId], world.Agenda.Outstanding.Where(step => step.What == Steps.Attack).Select(step => step.Subject));
        var scheduled = Assert.Single(world.Agenda.Outstanding, step => step.What == Steps.Attack && step.Subject == minion.ObjectId);
    ...
    public void ActivationKeepsCompiledTargetAndSnapshotController(string role)
    {
        ...
        var(runner, fields) = MutableEffectRunner("enemyAttacks", $$"""{"enemies":{"query":"villain"},"against":"{{role}}"}""", false);
        Card? source = null;
        Card? ally = null;
        var(_, world) = Playing(board =>
        {
        ...
        var action = Assert.Single(runner.Actions(world, 0), ability => ability.Card == source!.ObjectId);
        var occurrence = Occurrence.ForAttack(18, [Steps.TurnAction], world, CardCatalogData, ally!.ObjectId, world.Seats[1].IdentityCard.ObjectId, player: 0);
        ...
        Assert.Equal(role == "trigger.actor" ? ally.ObjectId : world.Seats[1].IdentityCard.ObjectId, attack.Character);
    ...
    {
        var(runner, fields) = MutableEffectRunner("enemyAttacks", """{"enemies":{"query":"minions"},"against":"engagedHero"}""", false);
        Card? source = null;
        Card? minion = null;
        var(_, world) = Playing(board =>
        {
            source = InPlay(board, AuthoredCards.AuntMay);
            minion = board.CreateCard(AuthoredCards.Shocker, board.AreaOf(DeckType.EngagedEnemiesArea, PlayArea.Of(1)));
            if (hero)
    ...
    public void ActivationOrderKeepsCompiledEnemiesAcrossTheChoice(bool attack, bool changeWhilePending)
    {
        ...
        var(runner, fields) = MutableEffectRunner(attack ? "enemyAttacks" : "enemySchemes", """{"enemies":{"query":"minionsEngagedWithYou"}}""", false);
        Card? source = null;
        Card? first = null;
        Card? second = null;
        ...
        void ChangeSelector() => fields["enemies"] = new AbilityValue.Map(new Dictionary<string, AbilityValue> { ["query"] = new AbilityValue.Word("villain") });
        ...
        {
            var placements = result.Events.OfType<FieldSet>().Where(change => change.Field == "k_threat").ToList();
    ...
    public void ActivationResumeKeepsTheCompiledDynamicFlag(bool dynamic, bool changeWhilePending)
    {
        ...
        var(runner, fields) = MutableEffectRunner("enemyAttacks", $$"""{"enemies":{"query":"minionsEngagedWithYou"},"dynamic":"{{(dynamic ? "true" : "false")}}"}""", false);
        Card? source = null;
        Card? first = null;
        var(game, world) = Playing(board =>
        {
            source = InPlay(board, AuthoredCards.AuntMay);
            first = board.CreateCard(AuthoredCards.Shocker, board.AreaOf(DeckType.EngagedEnemiesArea, PlayArea.Of(0)));
        }, hero: true, abilities: runner);
        var action = Assert.Single(game.Pending!.Affordances, option => option.AnchorId == source!.ObjectId);
        ...
        if (changeWhilePending)
            ChangeFlag();
        var newcomer = world.CreateCard(AuthoredCards.Shocker, world.AreaOf(DeckType.EngagedEnemiesArea, PlayArea.Of(0)));
        game.Resolve(Decision.Decline);
        if (dynamic)
        {
            Assert.Equal(Question.Defender, game.Pending!.Asking);
