...
public sealed class ActionAbilityDamageContinuationsDirectAttackDamageResumesItsFollowingEffectTests
{
    ...
    public void DirectAttackDamageResumesItsFollowingEffectAfterTheDefeatInterrupt()
    {
        ...
        var(game, world) = Playing(board =>
        {
            ...
            minion.TakeDamage(Damage.Health(board, CardCatalogData, minion) - 1);
    ...
    public void AssignedIndirectDamageResumesItsFollowingEffectAfterTheDefeatInterrupt()
    {
        ...
        var(game, world) = Playing(board =>
        {
            ...
            ally.TakeDamage(Damage.Health(board, CardCatalogData, ally) - 1);
        }, hero: true, abilities: runner);
        int held = world.Seats[0].Hand.Cards.Count;
        var action = Assert.Single(game.Pending!.Affordances, option => option.AnchorId == source!.ObjectId);
        game.Resolve(Decision.Take(action.Id));
