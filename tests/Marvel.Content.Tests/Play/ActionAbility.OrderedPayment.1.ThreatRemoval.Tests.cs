...
public sealed class ActionAbilityOrderedPaymentThreatRemovalTests
{
    [Fact]
    public void ThreatRemovalUsesTheInitiatingRunnersThreatAbilities()
    {
        ...
        runner.Act(world, Assert.Single(runner.Actions(world, 0), option => option.Card == source.ObjectId), [], []);
    ...
    public void DamageCostEligibilityUsesTheInitiatingRunnersProgram()
    {
        ...
        Assert.Throws<RulesNotImplementedException>(() => runner.Resolve(world, occurrence, new PendingAbility(source.ObjectId, AbilityType.Response, 0), [], []));
    ...
    public void PaymentCommitsTheSelectedCardsCapturedBeforeResourceGeneration()
    {
        ...
        var(program, runner) = PaymentRunner(AuthoredCards.AuntMay, """{"placeCounters":{"card":"you","counter":"test","count":1}}""", cost: """
        ...
        var payment = AbilityCostPayment.Prepare(world, source, 0, ability.Cost, [generator.ObjectId], chosen, program, probe);
        ...
        Assert.Single(events.OfType<FieldSet>(), change => change.Card == selected.ObjectId && change.Field == "is_exhaust");
    }
    ...
    public void PaymentDoesNotBindThisToASourceThatReenteredDuringResourceGeneration()
    {
        ...
        var(program, runner) = PaymentRunner(AuthoredCards.AuntMay, """{"placeCounters":{"card":"you","counter":"test","count":1}}""", cost: """{"seq":[{"spend":"Y"},{"exhaust":"this"}]}""");
        var(world, source) = OrderedPaymentBoard(runner);
        var generator = InPlay(world, "01091");
        ...
        var ability = Assert.Single(program.On(source.FaceId));
        var payment = AbilityCostPayment.Prepare(world, source, 0, ability.Cost, [generator.ObjectId], [], program, probe);
        var events = new List<GameEvent>();
        payment.Commit(runner, Steps.TurnAction, events);
        Assert.Equal(incarnation + 1, source.Incarnation);
        Assert.True(source.Ready);
        Assert.DoesNotContain(events.OfType<FieldSet>(), change => change.Card == source.ObjectId && change.Field == "is_exhaust");
    }
    ...
    public void CounterPaymentUsesTheExecutingInterpretersPool(string timing, bool uses)
    {
        ...
        string effect = timing == "Resource" ? """{"generate":"Y"}""" : """{"placeCounters":{"card":"you","counter":"test","count":1}}""";
        var runner = new AbilityRunner(AbilityCatalog.Parse($$$"""
            {"cards":[{"card":"01006",
        ...
        if (timing == "Resource")
            ...
            events.AddRange(runner.Resolve(world, new Occurrence(0, ["WhenActionTriggered"], Player: 0), new PendingAbility(source.ObjectId, AbilityType.Response, 0), [], []));
        else
            events.AddRange(runner.Act(world, Assert.Single(runner.Actions(world, 0), option => option.Card == source.ObjectId), [], []));
        ...
        Assert.Equal(timing == "Resource" ? 0 : 1, world.Seats[0].IdentityCard.Tokens.GetValueOrDefault("c_test"));
    }
    ...
    public void OrderedPaymentCombinesResourceComponentsIntoOneGeneratorUse()
    {
        ...
        var(program, runner) = PaymentRunner(AuthoredCards.AuntMay, """{"placeCounters":{"card":"you","counter":"test","count":1}}""", cost: """{"seq":[{"exhaust":"this"},{"spend":"Y"},{"spend":"Y"}]}""");
        var(world, source) = OrderedPaymentBoard(runner);
        var generator = InPlay(world, "01091");
        ...
        Assert.Single(events.OfType<FieldSet>(), change => change.Card == source.ObjectId && change.Field == "is_exhaust");
    }
    ...
    public void OrderedPaymentPreservesHealingResultsAndAuthoredPrimitiveOrder()
    {
        ...
        var runner = Runner(AuthoredCards.AuntMay, "Action", """{"placeCounters":{"card":"this","counter":"test","count":{"result":"healed"}}}""", cost: """
    ...
    public void EventDamageCostsReadLiveModifiersButAreNotAttacks(bool removeModifier)
    {
        ...
        var(program, runner) = PaymentRunner(AuthoredCards.Backflip, """{"placeCounters":{"card":"you","counter":"test","count":1}}""", cost: """{"seq":[{"spend":"Y"},{"dealDamage":{"cards":"you","amount":1}}]}""");
        var(world, _) = OrderedPaymentBoard(runner);
        var source = world.CreateCard(AuthoredCards.Backflip, world.Seats[0].Hand);
        var generator = InPlay(world, "01091");
        var modifier = new ContinuousEffect(EffectSource.LastingEffect, Kind: "eventDamage", Amount: 2, Card: generator.ObjectId, Affects: source.ObjectId, Lasts: new Duration(Uses: 1));
        world.Effects.Register(modifier);
        world.Effects.Register(new ContinuousEffect(EffectSource.LastingEffect, Kind: "attackDamage", Amount: 100, Card: generator.ObjectId, Affects: source.ObjectId, Lasts: new Duration(Uses: 1)));
        var probe = new PaymentResourceProbe(generator, "Y", () =>
        {
            if (removeModifier)
        ...
        var ability = Assert.Single(program.On(source.FaceId));
        var arrowPayment = AbilityCostPayment.Prepare(world, source, 0, ability.Cost, [generator.ObjectId], [], program, probe, resourcesPaidByEvent: true);
        var eventPayment = Assert.IsType<AbilityEventPayment>(AbilityEventPayment.Prepare(world, source, 0, [generator.ObjectId], ability.Effect, probe, allocations: [new ResourceAllocation(generator.ObjectId, Cost: 1, PaidAs: "Y")], additionalCost: ability.Cost));
        var events = new List<GameEvent>();
        ...
        int damaged = events.FindIndex(happened => happened is FieldSet change && change.Card == world.Seats[0].IdentityCard.ObjectId && change.Field == "health");
        Assert.True(moved >= 0);
    ...
    public void VariablePaymentReturnsTheDeclaredEnergyNotItsOverpayment(int declared)
    {
        ...
        var(program, runner) = PaymentRunner(AuthoredCards.AuntMay, """{"placeCounters":{"card":"this","counter":"test","count":{"result":"energy"}}}""", cost: """{"spendEnergyX":"Y"}""");
        var(world, source) = OrderedPaymentBoard(runner);
        var generator = InPlay(world, "01091");
        ...
        if (declared == 3)
        {
            Assert.Throws<RulesNotImplementedException>(() => AbilityCostPayment.Prepare(world, source, 0, ability.Cost, [generator.ObjectId], [], program, probe, values));
            Assert.Equal(before, world.Digest().Canonical());
            Assert.Equal(0, probe.Uses);
            return;
        }

        var payment = AbilityCostPayment.Prepare(world, source, 0, ability.Cost, [generator.ObjectId], [], program, probe, values);
