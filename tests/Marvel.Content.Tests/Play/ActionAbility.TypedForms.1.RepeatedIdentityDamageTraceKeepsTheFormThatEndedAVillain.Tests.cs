...
public sealed class ActionAbilityTypedFormsRepeatedIdentityDamageTraceKeepsTheFormThatEndedAVillainTests
{
    [Fact]
    ...
    public void FormChangeKeepsCompiledPlayerAndDestination(string wrapper)
    {
        ...
        static AbilityValue.Map Map(params (string Key, AbilityValue Value)[] entries) => new(entries.ToDictionary(entry => entry.Key, entry => entry.Value, StringComparer.Ordinal));
        var form = Map(("changeForm", new AbilityValue.Map(fields)));
        var effect = wrapper switch
        {
            "direct" => form,
            "choice" => Map(("choose", Map(("options", new AbilityValue.List([form, Map(("changeForm", Map(("player", new AbilityValue.Word("you")), ("to", new AbilityValue.Word("alter-ego"))))), ]))))),
            "repeated" => Map(("forEach", Map(("count", new AbilityValue.Number(2)), ("effect", form)))),
            "dependent" => Map(("then", Map(("effect", form), ("then", Map(("heal", Map(("card", new AbilityValue.Word("you")), ("amount", new AbilityValue.Number(1))))))))),
            _ => throw new ArgumentException("Unknown test wrapper", nameof(wrapper)),
        };
        var runner = new AbilityRunner(new AbilityBook([parsed.Abilities[0] with { Effect = AbilityNode.Of(effect) }], parsed.Authored));
        Card? source = null;
        var(game, world) = Playing(board =>
        {
            source = InPlay(board, AuthoredCards.AuntMay);
        ...
        Assert.True(Forms.In(world, world.Seats[1], CardCatalogData, Forms.Hero));
        Assert.True(Forms.In(world, world.Seats[0], CardCatalogData, Forms.AlterEgo));
