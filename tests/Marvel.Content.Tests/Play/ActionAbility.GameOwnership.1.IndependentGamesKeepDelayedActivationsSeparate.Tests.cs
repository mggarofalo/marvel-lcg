...
public sealed class ActionAbilityGameOwnershipIndependentGamesKeepDelayedActivationsSeparateTests
{
    [Theory]
    ...
    public void IndependentGamesKeepDelayedActivationsSeparate(bool shareRunner, bool reverseCompletion)
    {
        ...
        foreach (var(world, source, runner)in new[]
        {
            first,
            second
        }
        ...
        {
            world.Activation = new EnemyActivation(world.TheCardIn(DeckType.VillainArea)!.ObjectId, 0, Attacking: true, Id: 41);
        ...
        var boards = reverseCompletion ? new[]
        {
            second,
            first
        }
    ...
    public void IndependentGamesResumeTheirOwnChoiceResults(bool shareRunner)
    {
        ...
        foreach (var(game, world, source)in new[]
        {
            first,
            second
        }
