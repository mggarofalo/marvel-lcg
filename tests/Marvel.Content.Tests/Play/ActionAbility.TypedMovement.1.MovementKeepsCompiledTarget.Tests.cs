...
public sealed class ActionAbilityTypedMovementMovementKeepsCompiledTargetTests
{
    [Theory]
    ...
    {
        var(runner, fields) = MutableEffectRunner(operation, """{"titled":"Avengers Mansion"}""", false);
        Card? source = null;
        Card? target = null;
        ...
        var moved = Assert.Single(result.Events.OfType<CardsMoved>(), change => change.Cards.Any(card => card.Card == target.ObjectId));
    ...
    public void RandomMovementKeepsCompiledCountAndPlayerOrder(string operation)
    {
        ...
        var(runner, fields) = MutableEffectRunner(operation, $$"""{"player":"each","count":2{{host}}}""", false);
        Card? source = null;
        ...
        {
            while (seat.Hand.Cards.Count > 2)
                World.MoveToTop(seat.Hand.Cards[0], seat.Deck);
        ...
        var moved = result.Events.OfType<CardsMoved>().SelectMany(change => change.Cards).Select(card => world.Cards[card.Card]).ToList();
        ...
        Assert.All(moved, card =>
        {
            ...
            if (operation == "placeAtRandom")
                Assert.Equal(source!.ObjectId, card.Area.Host);
        });
    }
    ...
    public void TopDiscardKeepsCompiledCountAndPileOrder(string from)
    {
        ...
        var discard = from == "yourDeck" ? world.AreaOf(DeckType.DiscardPile, PlayArea.Of(0), cardOwner: 0) : world.AreaOf(DeckType.EncounterDiscardPile);
