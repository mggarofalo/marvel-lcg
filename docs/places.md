...
steps** across a large sample of recorded play, and the legacy engine does not
implement Fear No Evil. MARVEL-174 therefore keeps v2 frozen instead of adding
an empty or self-derived side channel. These rules are held against the
published text, and the tests name the rule each one comes from. Nine mutations
