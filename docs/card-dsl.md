...
| Card sources | `cardsIn { area | areas, kind, trait, title }` over `encounterDeck` and `encounterDiscardPile`; `enemiesWithTrait`; `titled`; `withoutAnotherCopyAttached`; `discardable` over a query; `minBy` / `maxBy` over a query, `by` `cost`, `attack`, or `printedHealth` |
| Dynamic amounts | `count`, `damageOn`, `countersOn`, `remainingHealth`, `startingHealth`, `powerAmount`, `discardedWithResource`, `printedResourceCountDiscarded`, `printedBoostIconsDiscarded`, `topEncounterDiscardBoostPlusOne`, `modified`, `perPlayer`; arithmetic `min`, `add`, `mul`; conditional `if` |
...
**Alteration `eachTime` is `{ effect, when, then }`.** The executable preceding
effect is currently an encounter-deck `discardTop`; it advances one card at a
time. After each discard, `that` binds the exact card, `cardSet` tests its
printed encounter set, and `dealEncounterCard` can move that same object to the
resolving player's facedown queue. A matching `then` finishes—including a
suspended choice—before the next discard. `activationIs` distinguishes attack
and scheme activations when printed text conditions a modifier on the current
activation.
...
An activation node may set `dynamic: "true"` when the printed instruction
names a collection whose membership can change while its activations resolve.
The runner re-evaluates the `enemies` query after each completed activation and
schedules every newly eligible physical enemy exactly once. If several newly
eligible enemies exist, the normal first-player ordering decision still
applies. This is deliberately opt-in: ordinary collection effects describe the
cards selected when the instruction begins, while Swarm Attack's sequence can
create another engaged Drone that the same instruction must then reach.

**`declareDefender` changes the imminent attack; it does not exhaust.** The
node takes one already-resolved `card`. A card that says "exhaust it and declare
it the defender" uses an `exhaust` instruction as well; a card that says
"without exhausting" uses only this node. Card-declared heroes make a basic
defense and apply DEF, while card-declared allies do not. Both may already be
exhausted because `rr:defend-defense.2.2` and `.3.3` expressly allow that
printed permission. The attack is board state during its initiation interrupt
so a `WhenAttackInitiated` ability can make the declaration before step 2.
...
The same slice adds `removeCounters` as a payable cost, `removeThreat` and
`advanceMainScheme: next` as actions, and the board queries `enemies` and
`schemes`. The counter type `allPurpose` is the engine's spelling for a generic
reference: it counts every `c_*` pool and can spend the sole occupied pool. If
several types are present, spending one raises until the protocol can ask the
player which physical counter to choose. An event action or
