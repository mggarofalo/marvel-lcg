...
**1,773 scenes, 58 shards, 631 MB gzipped**, pinned at
[`9642b29`](https://github.com/mggarofalo/marvel-lcg-corpus) with root hash
`b4e3946150a02dc6f77b2c9513d3ed5eaff209827734163b40c905d2c6fa212d`, generated by
engine `3446f61`. The manifest is `datasets/corpus/manifest.json`; see
[`datasets/corpus/UPSTREAM.md`](../datasets/corpus/UPSTREAM.md) for how it was
made and why it is three rounds rather than six.

Every scene in it reproduces every recorded step with every digest matching.
Twenty-seven scenes that reproduced exactly and *then* made the engine log an
error are excluded by name and are the C# engine's acceptance list.

CI still freezes a corpus it generated itself and then tampers with it, because
that gate tests the *mechanism* and must be able to fail. What the pin adds is
the other half: that the published bytes are the ones the manifest describes.
