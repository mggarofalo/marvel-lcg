...
`tests/Marvel.Core.Tests/Random/`, and the anchor is a standard rather than a
recording.

**ISO/IEC 14882 §rand.predef** defines `std::mt19937` as this generator seeded
with `5489`, and requires the **10,000th** consecutive draw to be
`4123659995`. Every conforming implementation reproduces it, so agreeing is
evidence about the algorithm and not about whoever wrote the test. It is also
sixteen refills of the 624-word state, so a generator that tempers correctly and
twists wrongly cannot reach it.

Around that anchor:

| what | why it is separate |
|---|---|
| the first ten draws of seed `5489` | localises a seeding bug the long vector only detects |
| the first draws of seeds `0`, `1`, `12345`, `2^32 - 1` | seed `0` is a seed, not a request for entropy; the top of the range is where a widening bug shows |
| draws 622–624 | the first refill boundary, both neighbours, so early and late are different failures |
| save and restore across a boundary | the state is 624 words *and* an index; omitting either diverges later rather than immediately |

Everything past `NextUInt32` is **ours** — no standard says how to reduce a word
to a bounded integer, which direction Fisher-Yates runs, or what an empty
shuffle costs — so those are tested as the pinned properties they are, including
the mixed sequence that interleaves them. A per-function match cannot hide a
stream-position error when one interleaved run has to land on the same word as a
raw generator.

**Two equivalent mutants are documented rather than tested**, because no test
can catch either: bit 0 of the `0x9D2C5680` tempering mask is unreachable
(`y << 7` never sets it), and `>=` for `==` in `Choice2`'s short-circuit cannot
differ once `ThrowIfGreaterThan` has rejected every larger `x`.
