...
`datasets/rng/vectors.json`, generated by
`py_src/tools/rng/emit_vectors.py` and checked by
`py_src/unit_test/test_rng.py`.

The fixture is the acceptance test for `MARVEL-8`: a C# implementation that
reproduces every vector is compatible, and one that does not is not. It covers
each function above independently, and a mixed sequence that interleaves them
so a per-function match cannot hide a stream-position error.
