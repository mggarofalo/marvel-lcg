...
Every replay step records a state digest — `World.CalculateCRC()` (`game/world/world_render.py:123`) — which `engine/controller/module/replay.py` compares against the recorded value on replay, printing a **key-by-key diff** on mismatch. Combined with the seeded MT19937, this means: replay the same corpus through both engines and get told exactly which card diverged at which step.
...
Two portability hazards, both logged as issues:

- `engine/lib/mt19937.py` `shuffle()` is not Fisher-Yates (it performs `10 * len` random swaps) and `randint` truncates a float division. Both must be ported bit-exactly, quirks included.
