...
| Linux server image | Immutable OCI digest plus Sigstore bundle | Keyless workflow identity; no desktop trust claim | Replay `engine-replay-v2`, RNG `mt19937-iso-cxx`, digest `state-digest-v2`, protocol `14` | Empty schema `3` volume | Start healthy with separate save and diagnostics volumes |
